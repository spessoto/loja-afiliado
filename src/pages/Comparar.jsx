...
export default function Comparar() {
  ...
  const precos = selected.map(p => Number(p.price_to) || Infinity);
            ...
            <div style={{ overflowX: "auto", border: "1px solid #E2E8F0", borderRadius: 12 }}>
              <table style={{ width: "100%", minWidth: 120 + selected.length * 240, borderCollapse: "separate", borderSpacing: 0 }}>
                    ...
                    {selected.map((p, i) => (
                      <th key={p.id} style={{ padding: "16px", minWidth: 220, verticalAlign: "top", borderBottom: "2px solid #E2E8F0" }}>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, textAlign: "center" }}>
                          ...
                          <div>
                            {Number(p.price_from) > Number(p.price_to) && <div style={{ font: "400 12.5px Inter", color: "#64748B", textDecoration: "line-through" }}>{formatBRL(p.price_from)}</div>}
                            <div style={{ font: "800 20px Montserrat", color: "#C84A00" }}>{formatBRL(p.price_to) || "Consulte"}</div>
                            {selected.length > 1 && precos[i] === menorPreco && <span style={{ display: "inline-block", marginTop: 4, padding: "2px 8px", borderRadius: 4, background: "#012746", color: "#fff", font: "700 10px Montserrat", letterSpacing: ".06em" }}>MENOR PREÇO</span>}
                          </div>
                ...
                <tbody>
                  ...
                  <tr>
                    <td style={labelCell}>Avaliação</td>
                    {selected.map(p => <td key={p.id} style={cell}>{p.rating_avg ? `${p.rating_avg}★ (${(Number(p.rating_count) || 0).toLocaleString("pt-BR")})` : "-"}</td>)}
                  </tr>
                  {linhas.map(l => (
                    <tr key={l.label}>
